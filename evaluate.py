...
if __name__ == "__main__":
    ...
    NUM_TREES_TO_EVAL = float('inf')
    if len(sys.argv) > 3:
        NUM_TREES_TO_EVAL = int(sys.argv[3])
    ...
    with open(sys.argv[2],'r') as in_file: 
        ...
        for i, dtree in enumerate(conll_reader(in_file)):
            if i == NUM_TREES_TO_EVAL: break
            words = dtree.words()
            pos = dtree.pos()
